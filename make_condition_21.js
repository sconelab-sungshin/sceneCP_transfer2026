function make_condition(relevant_ftr, random_cat_label, learning_set, n_round, transfer_set) {
    ...
    var relevant_ftr_idx = { //relevant feature index 
        group1: _.range(-9,0,2), // -9,-7,-5,-3,-1
        group2: _.range(9,0,-2)  // 9,7,5,3,1
    }; 
    var irrelevant_ftr_idx = {  //irrelevant feature index 
        group1:[
            _.range(-3, 4, 2), _.range(-5, 6, 2), _.range(-7, 8, 2),
            _.range(-9, 10, 2), _.range(-9, 10, 2)
        ],
        group2:[
            _.range(-3, 4, 2), _.range(-5, 6, 2), _.range(-7, 8, 2),
            _.range(-9, 10, 2), _.range(-9, 10, 2)
    ...
    // combining 
    var learning_stim = {
        group1: [[],[],[],[],[]], // # of sub-array = distance ragne (-9,-7,-5,-3,-1)
        group2: [[],[],[],[],[]]
    };
    ...
    //boundary위의 자극도 해당 
    var kf_testing_idx = {
        group1: [[-4,-2],[-6,-4],[-8,-6,-2],[-6,-4],[-8,-6,-2],[-6,-4],[-8,-6,-2],[-6,-4],[-8,-6,-2],[-6,-4],[-4,-2]],
        boundary: [[0],[0],[0],[0],[0]],
        group2:[[2,4],[4,6],[2,6,8],[4,6],[2,6,8],[4,6],[2,6,8],[4,6],[2,6,8],[4,6],[2,4]]
    };
    var nkf_testing_idx = {
