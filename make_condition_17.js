function make_condition(relevant_ftr, random_cat_label, learning_set, n_round, transfer_set) {
    ...
    var relevant_ftr_idx = { 
        group1: _.range(-7,0,2), 
        group2: _.range(7,0,-2)  
    }; 
    ...
    for (g of group_type){
        ...
        for(d of dist_range){
            ...
            // n_sample_perDist=0이면 전부 remain으로 보냄
            if(n_sample_perDist === 0){
                remain_learning_stim = remain_learning_stim.concat(shuffled_stim_perDist);
                continue;
            }
    
            var chunked_stim_perDist = _.chunk(shuffled_stim_perDist, n_sample_perDist);
            // 정확히 n_round개만 라운드에 배분
            for(r of round_num){
                if(chunked_stim_perDist[r]) rounded_learning_stim[g]['r'+(r+1)].push(chunked_stim_perDist[r]);
            }
        }
        remain_learning_stim = _.shuffle(remain_learning_stim.flat(Infinity));
        // remain을 라운드에 1개씩 배분
        for(var ri=0; ri<remain_learning_stim.length; ri++){
            var r_target = ri % n_round;
            rounded_learning_stim[g]['r'+(r_target+1)].push(remain_learning_stim[ri]);
        }
        for(r of round_num){
            rounded_learning_stim[g]['r'+(r+1)] = rounded_learning_stim[g]['r'+(r+1)].flat();      
        }
    ...
    var relevant_ftr_idx = { // relevant feature index
        group1: _.range(-5,-2,2),
        group2: _.range(5,2,-2)
    }; 
    ...
    // Test condition ----------------------------------------------------------------------------------- //
    var kf_testing_idx = {
        group1: [[-2],[-4,-2],[-6,-4,-2],[-4,-2],[-6,-4,-2],[-4,-2],[-6,-4,-2],[-4,-2],[-2]],
        boundary: [[0],[0],[0]], //수정 
        group2: [[2],[2,4],[2,4,6],[2,4],[2,4,6],[2,4],[2,4,6],[2,4],[2]]
    };
    var nkf_testing_idx = {
        group1: _.range(-8,9,2),      
        boundary: _.range(-4,5,4),
